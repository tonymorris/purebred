...
messageToMailView s msg =
  let
    body' = renderMarkup
      (preview (asMailView . mvSearchResult . _Just . to focusGetCurrent . _Just) s)
...
buildWordMarkup st (Line xs _ t) = foldr (go st) (t @? defaultAttr) xs
  where
    ...
    go Nothing (Match offset l _) m = markupSet (offset, l) textMatchHighlightAttr m
    go (Just (_, curMatch)) ma@(Match offset l _) m =
      if curMatch == ma
